...
const VIEWPORT_CSS = `
  * { box-sizing: border-box; }
  ...
  body { background: linear-gradient(135deg, #1a6fa8 0%, #4aadce 50%, #87ceeb 100%); overflow: hidden; }
  .uw-screen { display: flex; flex-direction: column; height: 100vh; width: 100vw; }
  .uw-menubar { display: flex; justify-content: space-between; align-items: center; padding: 6px 14px; background: rgba(255,255,255,0.72); backdrop-filter: blur(12px); font-size: 13px; }
  .uw-desktop { flex: 1; position: relative; }
  .uw-dock { display: flex; justify-content: center; gap: 10px; padding: 8px 14px 12px; }
  .uw-dock > .uw-box { display: flex; gap: 10px; padding: 8px 14px; background: rgba(255,255,255,0.75); backdrop-filter: blur(12px); border-radius: 14px; box-shadow: 0 4px 16px rgba(0,0,0,0.18); }
  ...
  .uw-window-chrome { position: absolute; background: #fff; border-radius: 10px; box-shadow: 0 12px 40px rgba(0,0,0,0.28); display: flex; flex-direction: column; overflow: hidden; }
  .uw-titlebar { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: #f0f0f0; border-bottom: 1px solid #ddd; }
